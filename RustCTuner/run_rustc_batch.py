...
BENCH_ROOT = PROJECT_ROOT / "Benchmarks" / "polybench-rs"


OUTPUT_ROOT = PROJECT_ROOT / "results" / "rustc_batch_results"
...
PROGRAMS = [
    ...
    "src/bin/bicg.rs",
    "src/bin/cholesky.rs",
    "src/bin/correlation.rs",
    "src/bin/covariance.rs",
    "src/bin/deriche.rs",
    "src/bin/doitgen.rs",
    "src/bin/durbin.rs",
    "src/bin/fdtd_2d.rs",
    "src/bin/floyd_warshall.rs",
    "src/bin/gemm.rs",
    "src/bin/gemver.rs",
    "src/bin/gesummv.rs",
    "src/bin/gramschmidt.rs",
    "src/bin/heat_3d.rs",
    "src/bin/jacobi_1d.rs",
    "src/bin/jacobi_2d.rs",
    "src/bin/lu.rs",
    "src/bin/ludcmp.rs",
    "src/bin/mvt.rs",
    "src/bin/nussinov.rs",
    "src/bin/seidel_2d.rs",
    "src/bin/symm.rs",
    "src/bin/syr2k.rs",
    "src/bin/syrk.rs",
    "src/bin/trisolv.rs",
...
def get_program_source_file(program_rel: str) -> Path:
...
def build_method_command(method: str, program_rel: str, run_dir: Path):
    ...
    if method == "RIO":
        ...
        cmd = maybe_add_common_rust_flags(cmd)
        return algo_dir, cmd
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Rustc batch runner with resumable repeat indexing"
    )
    parser.add_argument(
        "--num_repeats",
        type=int,
    ...
    append_text(
        master_log,
        f"\n========== Rustc Batch Run Start: {now_str()} | repeats {repeat_start}-{repeat_end} =========="
    )
    append_text(
        failed_log,
        f"\n========== Rustc Batch Failure Log Start: {now_str()} | repeats {repeat_start}-{repeat_end} =========="
    )
    ...
    append_text(
        master_log,
        f"========== Rustc Batch Run End: {now_str()} | repeats {repeat_start}-{repeat_end} ==========\n"
    )
