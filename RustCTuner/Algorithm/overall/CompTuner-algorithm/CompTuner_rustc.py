...
def get_objective_score_rustc(
    ...
    if current_time - state["last_log_time"] >= 20:
        best_mir_seq_str = ",".join(map(str, state["best_seq_so_far"][:total_mir])) if state["best_seq_so_far"] else ""
        best_llvm_seq_str = ",".join(map(str, state["best_seq_so_far"][total_mir:])) if state["best_seq_so_far"] else ""
