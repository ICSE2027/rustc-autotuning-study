...
def build_signature_by_scope(
    ...
    if tune_scope == "both":
        return build_joint_signature(mir_passes, llvm_flags, master_root, state)
        ...
        llvm_seq, llvm_pipeline = build_llvm_pipeline_from_state(
            master_root,
            llvm_flags,
            state["llvm"],
        )
        signature = ("", llvm_pipeline)
        return mir_seq, llvm_seq, mir_str, llvm_pipeline, signature
...
class RustcEvaluator:
    ...
    def evaluate(
        ...
        mir_seq, llvm_seq, mir_str, llvm_pipeline, signature = build_signature_by_scope(
            self.mir_passes,
            self.llvm_flags,
            self.master_root,
            state,
...
class GroupTunerRustc:
    ...
    def generate_candidate(
        ...
        for _ in range(max_tries):
            ...
            _, _, _, _, signature = build_signature_by_scope(
                self.mir_passes,
                self.llvm_flags,
...
if __name__ == "__main__":
    ...
    if args.tune_scope == "both":
        ...
        validate_joint_groups(mir_passes, [], mir_groups, [])
    elif args.tune_scope == "llvm":
        validate_joint_groups([], llvm_flags, [], llvm_groups)
    else:
        raise ValueError(f"Unknown tune_scope: {args.tune_scope}")

    if args.tune_scope == "both":
        active_group_count = len(mir_groups) + len(llvm_groups)
    elif args.tune_scope == "mir":
        active_group_count = len(mir_groups)
    elif args.tune_scope == "llvm":
        active_group_count = len(llvm_groups)
    else:
        raise ValueError(f"Unknown tune_scope: {args.tune_scope}")

    print(f"Total MIR groups: {len(mir_groups)}")
    print(f"Total LLVM groups: {len(llvm_groups)}")
    print(f"Active group count: {active_group_count}")
