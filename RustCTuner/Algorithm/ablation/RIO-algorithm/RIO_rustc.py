...
def sample_rustc_configuration(
    ...
    if tune_scope == "both":
        ...
        x = random.randint(0, 2 ** len(mir_passes) - 1)
        mir_seq = generate_random_conf(x, len(mir_passes))
        ...
        x = random.randint(0, 2 ** len(llvm_flags) - 1)
        llvm_seq = generate_random_conf(x, len(llvm_flags))
