...
def filter_related_flags_by_scope(
    related_flags: List[int],
    total_mir: int,
    total_llvm: int,
    tune_scope: str,
) -> List[int]:
    total_flags = total_mir + total_llvm

    cleaned = [x for x in related_flags if 0 <= x < total_flags]

    if tune_scope == "both":
        return cleaned

    if tune_scope == "mir":
        return [x for x in cleaned if x < total_mir]

    if tune_scope == "llvm":
        return [x - total_mir for x in cleaned if total_mir <= x < total_mir + total_llvm]
...
def get_objective_score_rustc(
    ...
    if current_time - state["last_log_time"] >= 20:
        best_mir_seq = ",".join(map(str, state["best_mir_seq_so_far"])) if state["best_mir_seq_so_far"] else ""
        best_llvm_seq = ",".join(map(str, state["best_llvm_seq_so_far"])) if state["best_llvm_seq_so_far"] else ""
...
if __name__ == "__main__":
    ...
    raw_related_flags = get_related_flags(raw_code, mir_passes, llvm_flags)
    related_flags_list = filter_related_flags_by_scope(
        related_flags=raw_related_flags,
        total_mir=total_mir,
        total_llvm=total_llvm,
        tune_scope=args.tune_scope,
    )

    if not related_flags_list:
        related_flags_list = list(range(total_flags))
        print("No related flags identified for active scope. Falling back to all active tunable passes.")

    print(f"Total MIR passes: {total_mir}")
    print(f"Total LLVM flags: {total_llvm}")
    print(f"Tune scope: {args.tune_scope}")
    print(f"Total Search Space Dimension: {total_flags}")
    print(f"Automatically identified {len(related_flags_list)} related active flags: {related_flags_list}")
